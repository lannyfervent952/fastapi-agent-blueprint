...
from src._core.infrastructure.admin.renderers import _build_column_defs, _build_row_data
...
class TestBuildColumnDefs:
    def test_returns_all_visible_columns(self):
        ...
        defs = _build_column_defs(config)
        assert len(defs) == 4
...
class TestBuildRowData:
    def test_masks_sensitive_fields(self):
        ...
        rows = _build_row_data(dtos, masked_fields={"password"})
    ...
    def test_converts_datetime_to_isoformat(self):
        dtos = [_make_sample_dto()]
        rows = _build_row_data(dtos, masked_fields=set())
    ...
    def test_empty_list(self):
        rows = _build_row_data([], masked_fields=set())
        assert rows == []
